...
WORDS_PATH = ROOT / "ege12_trainer_grouped_final.json"
...
FALLBACK_LETTERS = ["е", "и", "а", "я", "у", "ю", "о", "ё", "ы", "н", "т", "-"]
...
def category_pools(source_words: list[dict[str, Any]] | None = None) -> dict[str, dict[str, list[dict[str, Any]]]]:
    ...
    for word in source_words or WORDS:
        ...
        if not letter:
            continue
        group = f"{word['category']} / {word['rule_name']}"
        pools.setdefault(group, {}).setdefault(letter, []).append(word)
    return pools
