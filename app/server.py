...
LEGAL_DOCUMENT_RELEASE = "2026-08-31"
CURRENT_PRIVACY_POLICY_VERSION = os.environ.get("PRIVACY_POLICY_VERSION", LEGAL_DOCUMENT_RELEASE).strip()
CURRENT_CONSENT_VERSION = os.environ.get("CONSENT_VERSION", LEGAL_DOCUMENT_RELEASE).strip()
CURRENT_TERMS_VERSION = os.environ.get("TERMS_VERSION", LEGAL_DOCUMENT_RELEASE).strip()
...
def validate_document_seed_data(documents: list[dict[str, str]]) -> None:
    expected_types = {"privacy_policy", "personal_data_consent", "terms"}
    actual_types = {document["document_type"] for document in documents}
    if actual_types != expected_types:
        raise RuntimeError("Набор юридических документов неполон или содержит неизвестный тип.")

    forbidden_fragments = (
        "Текст будет заменен",
        "Текст является шаблоном",
        "[ФИО",
        "[Димитриев",
        "Редакция от [",
    )
    for document in documents:
        ...
        if document["version"] != LEGAL_DOCUMENT_RELEASE:
            raise RuntimeError(
                f"Версия юридического документа {document['document_type']} должна быть {LEGAL_DOCUMENT_RELEASE}."
            )
        if len(content) < 1_000:
            raise RuntimeError(f"Юридический документ {document['document_type']} похож на заглушку.")
        if "Димитриева Анастасия Владимировна" not in content:
            raise RuntimeError(f"В юридическом документе {document['document_type']} отсутствует ФИО оператора.")
        if any(fragment in content for fragment in forbidden_fragments):
            raise RuntimeError(f"В юридическом документе {document['document_type']} осталась заглушка.")


def seed_documents(con: sqlite3.Connection) -> None:
    documents = document_seed_data()
    validate_document_seed_data(documents)
    for document in documents:
        con.execute(
            "DELETE FROM documents WHERE document_type = ? AND version <> ?",
